    ...
    <main class="floating-shell" id="floating-shell" aria-label="WinPlate status">
      <section class="status-capsule">
        ...
        <div class="module codex-module no-drag">
          ...
          <div class="usage-track"><span class="usage-fill" style="width:${statusData.codex.remainingPct ?? 0}%"></span></div>
...
function dashboardContent(section) {
  const cards = `
    <div class="dashboard-grid">
      ...
      <article class="dashboard-card codex-card">
        <div class="card-icon">⌘</div><span>Codex Usage</span>
        ...
        <div class="large-track"><span style="width:${statusData.codex.remainingPct ?? 0}%"></span></div>
      </article>
  ...
  const content = {
    ...
    Codex: `<div class="page-heading"><p>CODEX</p><h1>Usage window</h1><span>${statusData.codex.windowHours}-hour allocation status.</span></div>${cards.split("</article>")[1]}</article>`,
  ...
  appRoot.innerHTML = `
    <div class="workspace">
      <aside class="sidebar">
        ...
        <nav>${sections.map((item) => `<button class="${item === currentSection ? "active" : ""}" data-section="${item}"><i>${item === "Dashboard" ? "⌂" : item === "GitHub" ? githubIcon : item === "Codex" ? "⌘" : item === "Heart" ? "♥" : "⚙"}</i>${item}</button>`).join("")}</nav>
