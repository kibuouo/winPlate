...
const networkDisplay = (typeof window !== "undefined" ? window : globalThis).WinPlateNetworkDisplay;
...
function networkSpeedLabel() {
  return `↓ ${networkDisplay.formatSpeedCompact(networkSpeed.downloadBytesPerSecond)}`;
}

function networkSpeedMarkup() {
  return `<span class="network-speed-arrow">↓</span><span class="network-speed-value">${networkDisplay.formatSpeedCompact(networkSpeed.downloadBytesPerSecond)}</span>`;
}

function networkStatusKind(status, downloadBytesPerSecond = 0, uploadBytesPerSecond = 0) {
  return networkDisplay.networkStatusKind(status, downloadBytesPerSecond, uploadBytesPerSecond);
}
...
function renderFloating() {
  ...
  bindSystemTooltip(networkModule, () => ({
    ...
    download: networkDisplay.formatNetworkSpeed(networkSpeed.downloadBytesPerSecond, false),
    upload: networkDisplay.formatNetworkSpeed(networkSpeed.uploadBytesPerSecond, false),
    latency: networkDisplay.formatLatency(networkSpeed.latencyMs)
  }));
...
function healthHeartRateRange(range = healthChartRange) {
  return window.WinPlateHealthTrend.heartRateRange(healthHistoryApi(), range);
}

function healthHeartRateSamples(range = healthChartRange, nowTimestamp = Date.now()) {
  return window.WinPlateHealthTrend.heartRateSamples(
    healthHistoryApi(),
    healthSyncStatus.heartRateHistory,
    range,
    nowTimestamp
  );
}
...
function healthHeartRateStats(samples) {
  return window.WinPlateHealthTrend.heartRateStats(healthHistoryApi(), samples);
}

function healthHeartRateAxisBounds(samples) {
  return window.WinPlateHealthTrend.axisBounds(healthHeartRateStats(samples));
}

function healthHeartRateAxisLabel(value, range) {
  return window.WinPlateHealthTrend.axisLabel(value, range);
}
...
function healthHeartRateChartPoints(
  ...
  return window.WinPlateHealthTrend.chartPoints(
    samples,
    {
      range,
      nowTimestamp,
    ...
    { historyApi: healthHistoryApi(), metric: healthMetric }
  );
}
...
function downsampleHeartRateSamples(samples, limit = 48) {
  return window.WinPlateHealthTrend.downsample(samples, limit);
}
