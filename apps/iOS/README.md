...
本版本不保存健康原始数据，不上传到互联网，也不申请写入权限。iPhone 只持久化最新一条待发送的健康概览，用于系统允许的后台重试；Mac 端保留当前运行期间收到的健康概览，Windows 端额外保留限定 7 天、去重后的心率采样摘要以支持心率趋势图。Windows 接收服务仅开放独立的 `8766` 端口，使用 WinPlate 生成的配对令牌；现有本地 API 仍只监听 `127.0.0.1:8765`。
