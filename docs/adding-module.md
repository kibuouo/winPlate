...
在 `apps/Windows/src/shared/moduleRegistry.js` 增加 `ModuleMeta`：
...
渲染模块必须通过 `apps/Windows/src/renderer/modules/contract.mjs` 校验，并实现：
...
- Electron IPC 边界登记在 `apps/Windows/src/main/modules/index.js`。
