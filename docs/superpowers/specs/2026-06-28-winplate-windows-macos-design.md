...
- Add a shared, pure menu-bar state module that normalizes the bounded temperature title and panel positioning inputs.
...
- Continue using the existing renderer for the main window and Windows desktop capsule. Expose the platform through the preload bridge so the main renderer can omit Windows chrome and apply macOS layout classes without Node access.
...
- Do not attach a persistent context menu. Left click toggles the popover; right click manually opens a menu containing Open WinPlate, Settings, Refresh, and Quit.
- Create the popover hidden at startup with the 320 × 420 preferred size defined by `2026-06-29-macos-menu-bar-design.md`, `vibrancy: "popover"`, active visual effects, a transparent frameless surface, system shadow, no resizing, and no taskbar entry.
...
- Keep the popover renderer alive while hidden so its existing 30-second refresh updates the menu bar temperature.
- The menu bar title and popover layout are superseded by `2026-06-29-macos-menu-bar-design.md`.

### macOS menu bar state

- The menu bar item, temperature title, panel contents, status points, and refresh behavior are defined by `2026-06-29-macos-menu-bar-design.md`.
- Send only a validated bounded temperature label from the panel renderer to the main process; the Tray adapter owns the native title update.
...
- Recreating a destroyed Tray, popover, or main window must be idempotent and must not register duplicate event handlers.
...
- Test temperature-title normalization, offline fallback, and bounded renderer-to-main payloads.
...
- On first launch, verify that the menu bar item appears and no macOS desktop floating capsule is created.
...
- Verify each preference applies live and survives restart, including menu bar enablement and launch at login.
- Verify current-data, partial-failure, and offline states while keeping all navigation and exit paths usable.
...
The same repository and shared data layer provide two deliberate desktop experiences: Windows retains its current floating-capsule product behavior, and macOS provides a working menu bar status center, native main window, and persisted preferences without a desktop floating capsule. All automated and macOS smoke checks pass, and no repository design concept images remain.
